...
public class VariantsController(CatalogDbContext context, IMapper mapper) : ControllerBase
{
    ...
    public async Task<ActionResult<VariantDto>> Create(CreateVariantDto dto)
    {
        ...
        var entity = new Variant { Name = dto.Name, DerivativeId = dto.DerivativeId, IsPopular = dto.IsPopular ?? false, IsImported = dto.IsImported ?? false, Code = await CodeGenerator.NextVariantCodeAsync(context) };
    ...
    public async Task<ActionResult> Update(int id, UpdateVariantDto dto)
    {
        ...
        if (codeNeedsUpdate)
        {
            entity.Code = await CodeGenerator.NextVariantCodeAsync(context);
